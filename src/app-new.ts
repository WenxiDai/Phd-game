...
import { SimpleGameTextEngine, GameTextEngine } from './gui/textEngine';
import { VariableStore } from './variableStore';
import { AleaRandomSource } from './utils/random';
...
class DirectUIProxy implements GuiActionProxy {
    ...
    constructor(private _textEngine: GameTextEngine) {
        // Text engine provided for localization and rendering
    }
    ...
    ensureUIElements(): void {
        ...
        this.messageWindow.style.zIndex = '1000';
    ...
    displayMessage(message: string, confirm: string, icon?: string, fx?: string): Promise<void> {
        ...
        const renderedMsg = this._textEngine.localizeAndRender(message);
        if (message === 'message.newMonth') {
            // Prepend the month-start image for the new-month event
            this.messageText.innerHTML =
                `<img src="images/month_start.png" alt="A New Month Start" style="display:block; position:relative; z-index:1001; max-width:100%; margin-bottom:10px;" />`
                + renderedMsg;
        } else {
            this.messageText.innerHTML = renderedMsg;
        }
        this.choicesContainer.innerHTML = '';

        // Create confirm button
        const confirmButton = document.createElement('button');
        confirmButton.innerHTML = this._textEngine.localizeAndRender(confirm);
    ...
    displayChoices(message: string, choices: Array<[string, number]>, icon?: string): Promise<number> {
        ...
        // Display the message
        this.messageText.innerHTML = this._textEngine.localizeAndRender(message);
        this.choicesContainer.innerHTML = '';

        // Create choice buttons
        choices.forEach(([choiceText, choiceId]) => {
            const choiceButton = document.createElement('button');
            choiceButton.innerHTML = this._textEngine.localizeAndRender(choiceText);
...
class TemplateGameApp {
    ...
    constructor(config: AppConfig) {
        ...
        // Ensure we pass a string seed to the random source
        const seedValue = config.initialRandomSeed ?? '';
        const randomSource = new AleaRandomSource(seedValue);
        const textEngine = new SimpleGameTextEngine(this.localizer, variableStore, randomSource);
        this.uiProxy = new DirectUIProxy(textEngine);
    ...
    async initialize(): Promise<void> {
        ...
        try {
            ...
            // (Background music is now handled in the HTML after user interaction.)
        } catch (error) {
    ...
    private handleGameEnd(event: GameEndEvent): void {
        ...
        if (gameOverScreen && gameScreen) {
            ...
            if (gameOverMessage) {
                ...
                if (event.state === EndGameState.Win) {
                    const gameWonBgm = document.getElementById('game-won-music');
                    if (gameWonBgm instanceof HTMLAudioElement) {
                        gameWonBgm.volume = 0.5;
                        gameWonBgm.play().catch(err => console.error('Game won BGM play failed:', err));
                    }
                } else {
                    const gameOverBgm = document.getElementById('game-over-music');
                    if (gameOverBgm instanceof HTMLAudioElement) {
                        gameOverBgm.volume = 0.5;
                        gameOverBgm.play().catch(err => console.error('Game over BGM play failed:', err));
                    }
                }
            }
        }
