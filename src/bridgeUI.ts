...
import { GameTextEngine } from './gui/textEngine';
...
export class TemplateUIProxy implements GuiActionProxy {
    ...
    private _textEngine: GameTextEngine;

    constructor(textEngine: GameTextEngine) {
    ...
    displayMessage(message: string, confirm: string, icon?: string, fx?: string): Promise<void> {
        ...
        if (messageText && choicesContainer) {
            // Display the message
            messageText.innerHTML = this._textEngine.localizeAndRender(message);
